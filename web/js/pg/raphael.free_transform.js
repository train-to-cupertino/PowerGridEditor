...
(function (root, factory) {
	...
	Raphael.fn.freeTransform = function(subject, options, callback) {
		...
		ft.showHandles = function() {
			...
			draggables.map(function(draggable) {
				draggable.drag(function(dx, dy) {
					...
					/*
					// This strings replace on edit placed below
					...
					*/
					...
					if ( ft.opts.snap.drag ) {
						ft.attrs.translate.x = Math.round((ft.o.translate.x + dx) / ft.opts.snap.drag) * ft.opts.snap.drag;
						ft.attrs.translate.y = Math.round((ft.o.translate.y + dy) / ft.opts.snap.drag) * ft.opts.snap.drag;
					}
					else {
						ft.attrs.translate.x = ft.o.translate.x + dx;
						ft.attrs.translate.y = ft.o.translate.y + dy;					
		...
		ft.setOpts = function(options, callback) {
			...
			// Edited by Den
			ft.opts.snap = {
				drag:   ft.opts.snap.drag,
				rotate: ft.opts.snap.rotate,
				scale:  ft.opts.snap.scale
			};

			ft.opts.snapDist = {
				drag:   ft.opts.snapDist.drag,
				rotate: ft.opts.snapDist.rotate,
				scale:  ft.opts.snapDist.scale
			};
			*/
