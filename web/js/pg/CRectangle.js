class Rectangle extends GraphicElement {
	...
	constructor(_pg) {
		...
		this.id = PGElement.getUniqueID("rect_");
	}
	...
	render() {
		...
		//this.graphics = new PGElemGraphics(this.pg.paper, this.pg.groups.rectangles);
		this.graphics = new PGElemGraphics(this.pg.paper, this.pg.groups.figures);
		
		this.graphics.add("body", 
			this.pg.paper.rect(0, 0, 100, 100)
	...
	openParamForm() {
		...
		$('<input>', {
			type: "button",
			value: "Удалить этот прямоугольник",
			class: "delete_elem_btn",
			on: {
				click: function(event){
					var _this_elem = _this;
					...
					_this.pg.actionManager.addAction(
						...
						function(){
							//_this.pg.addRectangle(_this_elem);
							_this.pg.addFigure(_this_elem);
						},
						
						function() {
							//_this.pg.deleteRectangle(_this_elem.id);
					...
					*/
					...
					//_this.pg.deleteRectangle(_this.id);
